<?php
/**
* optimizeMember's Tag protection routines *( for specific Tags )*.
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_ptags_sp"))
	{
		/**
		* optimizeMember's Tag protection routines *( for specific Tags )*.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_ptags_sp
			{
				/**
				...
				public static function check_specific_ptag_level_access ($_tag = FALSE, $check_user = TRUE)
					{
						...
						if (!$excluded && !empty ($tag_id) && !empty ($tag_slug) && !empty ($tag_name) && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_options_page"])
							{
								$tag_uri = c_ws_plugin__optimizemember_utils_urls::parse_uri (get_tag_link ($tag_id)); /* Get a full valid URI for this Tag. */
								/**/
								if (!c_ws_plugin__optimizemember_systematics_sp::is_wp_systematic_use_specific_page (null, $tag_uri)) /* Do NOT touch WordPress Systematics. */
									{
										...
										/**/
										else if (!c_ws_plugin__optimizemember_systematics_sp::is_systematic_use_specific_page (null, $tag_uri)) /* Never restrict Systematics. However, there is 1 exception above. */
											{
