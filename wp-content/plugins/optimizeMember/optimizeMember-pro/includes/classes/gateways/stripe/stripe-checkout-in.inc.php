	...
	class c_ws_plugin__optimizemember_pro_stripe_checkout_in
	{
		...
		public static function stripe_checkout()
		{
			...
			)
			{
				...
				if(!c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($post_vars['attr'])) // Attr errors?
				{
					...
					{
						unset($_POST['optimizemember_pro_stripe_checkout']['card_token']); // These are good one-time only.
						...
						if($use_subscription && is_user_logged_in() && is_object($user = wp_get_current_user()) && ($user_id = $user->ID))
						{
							...
							if(!$global_response)
								if(($post_vars['attr']['tp'] && $cost_calculations['trial_total'] > 0) || (!$post_vars['attr']['tp'] && $cost_calculations['total'] > 0))
								{
									...
									else if(!is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::set_customer_card_token($stripe_customer->id, $post_vars['card_token'])))
										$global_response = array('response' => $stripe_customer, 'error' => TRUE);

									else if(!is_object($stripe_charge = c_ws_plugin__optimizemember_pro_stripe_utilities::create_customer_charge($stripe_customer->id, ($post_vars['attr']['tp'] && $cost_calculations['trial_total'] > 0) ? $cost_calculations['trial_total'] : $cost_calculations['total'], $cost_calculations['cur'], $cost_calculations['desc'])))
								...
								if($cost_calculations['total'] > 0) // NOTE: it is s2Member's job to stop non-recurring subscriptions.
								{
									if(!is_object($stripe_plan = c_ws_plugin__optimizemember_pro_stripe_utilities::get_plan($plan_attr)))
										$global_response = array('response' => $stripe_plan, 'error' => TRUE);
									        ...
									        && !is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer($user_id, $user->user_email, $post_vars['first_name'], $post_vars['last_name']))
									) $global_response = array('response' => $stripe_customer, 'error' => TRUE);

									else if((!isset($stripe_customer) || !is_object($stripe_customer))
									        && !is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::set_customer_card_token($stripe_customer->id, $post_vars['card_token']))
									) $global_response = array('response' => $stripe_customer, 'error' => TRUE);

									else if(!is_object($stripe_subscription = c_ws_plugin__optimizemember_pro_stripe_utilities::create_customer_subscription($stripe_customer->id, $stripe_plan->id)))
						...
						else if($use_subscription && !is_user_logged_in()) // Create a new account.
						{
								...
								if($cost_calculations['total'] > 0) // NOTE: it is s2Member's job to stop non-recurring subscriptions.
								{
									if(!is_object($stripe_plan = c_ws_plugin__optimizemember_pro_stripe_utilities::get_plan($plan_attr)))
										$global_response = array('response' => $stripe_plan, 'error' => TRUE);
									        ...
									        && !is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer(0, $post_vars['email'], $post_vars['first_name'], $post_vars['last_name']))
									) $global_response = array('response' => $stripe_customer, 'error' => TRUE);
							...
							if(!$global_response) // No errors thus far?
							{
								...
								$create_user['user_pass']  = wp_generate_password(); // Which may fire `c_ws_plugin__optimizemember_registrations::generate_password()`.
						...
						else if(!$use_subscription && is_user_logged_in() && is_object($user = wp_get_current_user()) && ($user_id = $user->ID))
						{
							...
							if(!$global_response)
								if($cost_calculations['total'] > 0)
								{
									...
									else if(!is_object($stripe_charge = c_ws_plugin__optimizemember_pro_stripe_utilities::create_customer_charge($stripe_customer->id, $cost_calculations['total'], $cost_calculations['cur'], $cost_calculations['desc'])))
										$global_response = array('response' => $stripe_charge, 'error' => TRUE);
