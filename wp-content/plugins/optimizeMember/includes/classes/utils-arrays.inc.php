...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_utils_arrays"))
	{
		/**
		* Array utilities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_utils_arrays
			{
				/**
								...
								foreach ($array as $value)
									{
										if (is_array ($value) /* Recursive function call. */)
											{
												if (c_ws_plugin__optimizemember_utils_arrays::in_regex_array ($string, $value))
													return true;
											}
										else if (is_string ($value) /* Must be a string. */)
											{
				...
				public static function remove_0b_strings ($array = FALSE)
					{
						...
						foreach ($array as $key => &$value)
							{
								if (is_array ($value) /* Recursive function call here. */)
									$value = c_ws_plugin__optimizemember_utils_arrays::remove_0b_strings ($value);
								/**/
