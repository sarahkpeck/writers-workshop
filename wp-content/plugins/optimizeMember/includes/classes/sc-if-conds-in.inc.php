				...
				public static function sc_if_conditionals ($attr = FALSE, $content = FALSE, $shortcode = FALSE)
					{
						...
						/**/
						$attr = c_ws_plugin__optimizemember_utils_strings::trim_qts_deep ((array)$attr); /* Force array; trim quote entities. */
						/**/
							...
							if (preg_match ("/^(&&|&amp;&amp;|&#038;&#038;|AND|\|\||OR|[\!\=\<\>]+)$/i", $attr_value))
								{ /* Stick with AND/OR. Ampersands are corrupted by the Visual Editor. */
									...
									if (preg_match ("/^[\!\=\<\>]+$/i", $attr_value)) /* Error on these operators. */
										{
											trigger_error ("opmIf, invalid operator [ " . $attr_value . " ]. Simple Conditionals cannot process operators like ( == != <> ). Please use Advanced (PHP) Conditionals instead.", E_USER_ERROR);
						...
						if (is_array ($logicals) && !empty ($logicals) && count (array_unique ($logicals)) > 1)
							{
								trigger_error ("opmIf, AND/OR malformed conditional logic. It's NOT possible to mix logic using AND/OR combinations. You MUST stick to one type of logic or another. If both types of logic are needed, you MUST use two different Shortcode expressions. Or, use Advanced (PHP) Conditionals instead.", E_USER_ERROR);
								return ""; /* Return now; empty string in this case. */
							}
						/**/
														...
														if (is_array ($args = preg_split ("/[;,]+/", $attr_args, 0, PREG_SPLIT_NO_EMPTY))) /* Convert all arguments into an array. And take note; possibly into an empty array. */
															{
																if (!is_multisite () || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () || is_main_site () || in_array (strtolower ($conditional), $blog_farm_safe))
																	...
																	{
																		trigger_error ("opmIf, unsafe conditional function [ " . $attr_value . " ]", E_USER_ERROR);
																		return ""; /* Return now; empty string in this case. */
																	}
															}
														else
															{
																trigger_error ("opmIf, conditional args are NOT an array [ " . $attr_value . " ]", E_USER_ERROR);
																return ""; /* Return now; empty string in this case. */
															}
													}
												else
													{
														trigger_error ("opmIf, unsafe conditional args [ " . $attr_value . " ]", E_USER_ERROR);
														return ""; /* Return now; empty string in this case. */
													}
											}
										else
											{
												trigger_error ("opmIf, malformed conditional [ " . $attr_value . " ]", E_USER_ERROR);
												return ""; /* Return now; empty string in this case. */
											}
									}
								/* Supports nested Shortcodes. */
								return do_shortcode (apply_filters ("ws_plugin__optimizemember_sc_if_conditionals", (($condition_failed) ? "" : $content), get_defined_vars ()));
							}
						/**/
						else if ($conditional_logic === "OR") /* This is the OR variation. This routine analyzes conditionals using OR logic, instead of AND logic. */
							{
								...
								/* Supports nested Shortcodes. */
								return do_shortcode (apply_filters ("ws_plugin__optimizemember_sc_if_conditionals", (($condition_succeeded) ? $content : ""), get_defined_vars ()));
							}
					}
			}
	}
