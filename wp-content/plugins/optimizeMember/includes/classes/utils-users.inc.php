				...
				public static function get_user_custom_with ($subscr_or_txn_id = FALSE, $os0 = FALSE)
					{
						...
						if ($subscr_or_txn_id && $os0) /* This case includes some additional routines that can use the ``$os0`` value. */
							{
								if (($q = $wpdb->get_row ("SELECT `user_id` FROM `" . $wpdb->usermeta . "` WHERE (`meta_key` = '" . $wpdb->prefix . "optimizemember_subscr_id' OR `meta_key` = '" . $wpdb->prefix . "optimizemember_first_payment_txn_id') AND (`meta_value` = '" . $wpdb->escape ($subscr_or_txn_id) . "' OR `meta_value` = '" . $wpdb->escape ($os0) . "') LIMIT 1"))/**/
							...
							{
								if (($q = $wpdb->get_row ("SELECT `user_id` FROM `" . $wpdb->usermeta . "` WHERE (`meta_key` = '" . $wpdb->prefix . "optimizemember_subscr_id' OR `meta_key` = '" . $wpdb->prefix . "optimizemember_first_payment_txn_id') AND `meta_value` = '" . $wpdb->escape ($subscr_or_txn_id) . "' LIMIT 1")))
									if (($custom = get_user_option ("optimizemember_custom", $q->user_id)))
										return $custom;
							}
						/**/
