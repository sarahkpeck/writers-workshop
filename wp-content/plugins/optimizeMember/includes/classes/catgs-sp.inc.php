<?php
/**
* optimizeMember's Category protection routines *( for specific Categories )*.
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_catgs_sp"))
	{
		/**
		* optimizeMember's Category protection routines *( for specific Categories )*.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_catgs_sp
			{
				/**
				...
				public static function check_specific_catg_level_access ($cat_id = FALSE, $check_user = TRUE)
					{
						...
						if (!$excluded && is_numeric ($cat_id) && ($cat_id = (int)$cat_id) && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_options_page"])
							{
								$cat_uri = c_ws_plugin__optimizemember_utils_urls::parse_uri (get_category_link ($cat_id)); /* Get a full valid URI for this Category. */
								/**/
								if (!c_ws_plugin__optimizemember_systematics_sp::is_wp_systematic_use_specific_page (null, $cat_uri)) /* Do NOT touch WordPress Systematics. */
									{
										...
										/**/
										else if (!c_ws_plugin__optimizemember_systematics_sp::is_systematic_use_specific_page (null, $cat_uri)) /* Never restrict Systematics. However, there is 1 exception ^. */
											{
												for ($n = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n >= 0; $n--) /* Category Level restrictions. Go through each Level. We also check nested Categories. */
													{
														...
														/**/
														else if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_catgs"] && in_array ($cat_id, ($catgs = preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_catgs"]))) && (!$check_user || !$user || !$user->has_cap ("access_optimizemember_level" . $n)))
															return apply_filters ("ws_plugin__optimizemember_check_specific_catg_level_access", array ("optimizemember_level_req" => $n), get_defined_vars ());
														/**/
														else if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_catgs"]) /* Check Category ancestry. */
