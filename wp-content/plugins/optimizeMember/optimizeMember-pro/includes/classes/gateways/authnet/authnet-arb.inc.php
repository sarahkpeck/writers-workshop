...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_authnet_arb"))
	{
		/**
		* Authorize.Net ARB.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_authnet_arb
			{
				/**
				...
				* To increase, use: ``add_filter ("ws_plugin__optimizemember_pro_arb_service_ipns_per_process");``.
				*
				* @package optimizeMember\AuthNet
				* @since 1.5
				*
				...
				public static function authnet_arb_service ($vars = FALSE)
					{
						...
						if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_login_id"])
							{
								$scan_time = apply_filters ("ws_plugin__optimizemember_pro_arb_service_status_scan_time", strtotime ("-1 day"), get_defined_vars ());
								$per_process = apply_filters ("ws_plugin__optimizemember_pro_arb_service_ipns_per_process", $vars["per_process"], get_defined_vars ());
								/**/
								if (is_array ($objs = $wpdb->get_results ("SELECT `user_id` AS `ID` FROM `" . $wpdb->usermeta . "` WHERE `meta_key` = '" . $wpdb->prefix . "optimizemember_subscr_gateway' AND `meta_value` = 'authnet' AND `user_id` NOT IN(SELECT `user_id` FROM `" . $wpdb->usermeta . "` WHERE `meta_key` = '" . $wpdb->prefix . "optimizemember_last_status_scan' AND `meta_value` > '" . esc_sql ($scan_time) . "')")))
									{
												...
												if (($user_id = $obj->ID) && ($counter = (int)$counter + 1)) /* Update counter. Only run through X records; given by $per_process. */
													{
														...
														/**/
														if (($authnet = array ("x_method" => "status")) && ($authnet["x_subscription_id"] = $subscr_id = get_user_option ("optimizemember_subscr_id", $user_id)))
															{
																if (!get_user_option ("optimizemember_auto_eot_time", $user_id) && is_array ($ipn_sv = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_vars (false, $subscr_id)))
																	{
																		if (($authnet = c_ws_plugin__optimizemember_pro_authnet_utilities::authnet_arb_response ($authnet)) && empty ($authnet["__error"]) && $authnet["subscription_status"] && is_array ($authnet["arb_ipn_signup_vars"] = $ipn_sv))
																			{
																				if (preg_match ("/^expired$/i", $authnet["subscription_status"])) /* Expired? */
																					{
																						$authnet["optimizemember_log"][] = "Authorize.Net ARB/IPN processed on: " . date ("D M j, Y g:i:s a T");
																						/**/
																						$authnet["optimizemember_log"][] = "Authorize.Net transaction identified as ( `SUBSCRIPTION EXPIRATION` ).";
																						$authnet["optimizemember_log"][] = "IPN reformulated. Piping through optimizeMember's core/standard PayPal processor as `txn_type` ( `subscr_eot` ).";
																				...
																				/**/
																				else if (preg_match ("/^(suspended|canceled|terminated)$/i", $authnet["subscription_status"]))
																					{
																						$authnet["optimizemember_log"][] = "Authorize.Net ARB/IPN processed on: " . date ("D M j, Y g:i:s a T");
																						/**/
																						$authnet["optimizemember_log"][] = "Authorize.Net transaction identified as ( `SUBSCRIPTION " . strtoupper ($authnet["subscription_status"]) . "` ).";
																						$authnet["optimizemember_log"][] = "IPN reformulated. Piping through optimizeMember's core/standard PayPal processor as `txn_type` ( `subscr_cancel` ).";
																						$authnet["optimizemember_log"][] = "Please check PayPal IPN logs for further processing details.";
																						/**/
																						$processing = $processed = true;
																						$ipn = array (); /* Reset. */
																				...
																				/**/
																				else if (!$processed) /* If nothing was processed, here we add a message to the logs indicating the status; which is being ignored by optimizeMember. */
																					$authnet["optimizemember_log"][] = "Ignoring this ARB/Status ( `" . $authnet["subscription_status"] . "` ). It does NOT require any action on the part of optimizeMember.";
																				/**/
