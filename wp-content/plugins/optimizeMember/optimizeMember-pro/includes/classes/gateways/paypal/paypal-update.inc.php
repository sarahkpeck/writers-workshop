...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_paypal_update"))
	{
		/**
		* PayPal Update Form processing.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_update
			{
				/**
				...
				*/
				public static function paypal_update ()
					{
						if (!empty ($_POST["optimizemember_pro_paypal_update"])) /* Call inner routine? */
							{
								if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_payflow_api_username"])
									return c_ws_plugin__optimizemember_pro_paypal_update_pf_in::paypal_update();
								/**/
								return c_ws_plugin__optimizemember_pro_paypal_update_in::paypal_update ();
							}
					}
			}
	}
