...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_utils_dirs"))
	{
		/**
		* Directory utilities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_utils_dirs
			{
				/**
				...
				*/
				public static function strip_dir_app_data ($path = FALSE)
					{
						return preg_replace ("/\/app_data$/", "", c_ws_plugin__optimizemember_utils_dirs::n_dir_seps ((string)$path));
					}
				/**
				...
				*/
				public static function basename_dir_app_data ($path = FALSE)
					{
						$path = preg_replace ("/\/app_data$/", "", c_ws_plugin__optimizemember_utils_dirs::n_dir_seps ((string)$path), 1, $app_data);
						/**/
				...
				*/
				public static function doc_root_path ($path = FALSE)
					{
						$doc_root = c_ws_plugin__optimizemember_utils_dirs::n_dir_seps ($_SERVER["DOCUMENT_ROOT"]);
						/**/
						return preg_replace ("/^" . preg_quote ($doc_root, "/") . "/", "", c_ws_plugin__optimizemember_utils_dirs::n_dir_seps ((string)$path));
					}
				/**
						...
						if ( /* Initialize/validate. */!($rel_path = array ()) && is_string ($from) && strlen ($from) && is_string ($to) && strlen ($to))
							{
								...
								/**/
								$from = c_ws_plugin__optimizemember_utils_dirs::n_dir_seps ($from); /* Normalize directory separators now. */
								$to = c_ws_plugin__optimizemember_utils_dirs::n_dir_seps ($to); /* Normalize directory separators here too. */
								/**/
