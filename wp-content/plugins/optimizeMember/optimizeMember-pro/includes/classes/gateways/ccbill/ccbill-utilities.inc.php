...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_ccbill_utilities"))
	{
		/**
		* ccBill utilities.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_ccbill_utilities
			{
				/**
				...
				public static function ccbill_link_gen ($vars = FALSE)
					{
						...
						/**/
						$vars["formDigest"] = md5 ($digest_vars . $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_salt_key"]);
						/**/
				...
				* @return str A 3 character Currency Code, for use with optimizeMember. Defaults to `USD` *( i.e. `840` )*.
				*/
						...
						if ((isset ($_REQUEST["optimizemember_pro_ccbill_return"]) && strlen ($_REQUEST["optimizemember_pro_ccbill_return"])) || (isset ($_REQUEST["optimizemember_pro_ccbill_notify"]) && strlen ($_REQUEST["optimizemember_pro_ccbill_notify"])))
							{
								...
								if ($postvars["responseDigest"] === md5 ($approval_digest_vars . $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_salt_key"]))
									return $postvars;
								/**/
								else if ($postvars["responseDigest"] === md5 ($denial_digest_vars . $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_salt_key"]))
									return $postvars;
								/**/
								else /* Nope. */
									return false;
