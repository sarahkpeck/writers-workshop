<?php
/**
* optimizeMember's Page protection routines *( for specific Pages )*.
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pages_sp"))
	{
		/**
		* optimizeMember's Page protection routines *( for specific Pages )*.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_pages_sp
			{
				/**
				...
				public static function check_specific_page_level_access ($page_id = FALSE, $check_user = TRUE)
					{
						...
						if (!$excluded && is_numeric ($page_id) && ($page_id = (int)$page_id) && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_options_page"])
							{
								$page_uri = c_ws_plugin__optimizemember_utils_urls::parse_uri (get_page_link ($page_id)); /* Get a full valid URI for this Page now. */
								/**/
								if (!c_ws_plugin__optimizemember_systematics_sp::is_wp_systematic_use_specific_page ($page_id, $page_uri)) /* Do NOT touch WordPress Systematics. */
									{
										...
										/**/
											...
											return apply_filters ("ws_plugin__optimizemember_check_specific_page_level_access", array ("optimizemember_level_req" => 0), get_defined_vars ());
										/**/
										else if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["file_download_limit_exceeded_page"] && $page_id === (int)$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["file_download_limit_exceeded_page"] && (!$check_user || !$user || !$user->has_cap ("access_optimizemember_level0")) && $page_id !== (int)$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_options_page"])
											return apply_filters ("ws_plugin__optimizemember_check_specific_page_level_access", array ("optimizemember_level_req" => 0), get_defined_vars ());
										/**/
										else if (!c_ws_plugin__optimizemember_systematics_sp::is_systematic_use_specific_page ($page_id, $page_uri)) /* However, there are 3 exceptions above. */
											{
