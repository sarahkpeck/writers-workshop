...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_readmes"))
	{
		/**
		* Readme file parsing.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_readmes
			{
				/**
				...
				public static function parse_readme ($specific_path = FALSE, $specific_section = FALSE, $_blank_targets = TRUE, $process_wp_syntax = FALSE)
					{
						...
						do_action ("ws_plugin__optimizemember_before_parse_readme", get_defined_vars ());
						...
						if (file_exists ($path)) /* Give hooks a chance. */
							{
								...
								$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($rm, "UTF-8", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["mb_detection_order"]) : $rm;
								...
								if ($specific_section) /* If we are ONLY parsing a specific section. This is a very useful way of pulling details out. */
									{
										...
										$readme .= '</div>' . "\n";
										/**/
										return apply_filters ("ws_plugin__optimizemember_parse_readme", $readme, get_defined_vars ());
									}
								else /* Otherwise, we're going for the entire readme file. Here we have lots of work to do. */
									{
										...
										$rm = stripslashes (preg_replace ($r3, "'$1$2$3$4$5'.c_ws_plugin__optimizemember_readmes::_parse_readme_specs('$6').'$7'", $rm, 1));
				...
				*/
				public static function _parse_readme_specs ($str = FALSE)
					{
						do_action ("_ws_plugin__optimizemember_before_parse_readme_specs", get_defined_vars ());
						/**/
						...
						/**/
						return apply_filters ("_ws_plugin__optimizemember_parse_readme_specs", $str, get_defined_vars ());
					}
				/**
				...
				public static function parse_readme_value ($key = FALSE, $specific_path = FALSE)
					{
						...
						/**/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_parse_readme_value", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if (!empty ($readme[$path]) || file_exists ($path))
							{
								if (empty ($readme[$path])) /* If not already opened. */
									{
										...
										$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($readme[$path], "UTF-8", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["mb_detection_order"]) : $readme[$path];
								...
								/**/
								return apply_filters ("ws_plugin__optimizemember_parse_readme_value", ((isset ($m[5]) && strlen ($m[5] = trim ($m[5]))) ? $m[5] : false), get_defined_vars ());
							}
						else /* Nope. */
							return false;
					}
