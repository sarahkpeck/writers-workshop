...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_authnet_cancellation_in"))
	{
		/**
		* Authorize.Net Cancellation Form handler ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_authnet_cancellation_in
			{
				/**
				...
				public static function authnet_cancellation ()
					{
						if (!empty ($_POST["optimizemember_pro_authnet_cancellation"]["nonce"]) && ($nonce = $_POST["optimizemember_pro_authnet_cancellation"]["nonce"]) && wp_verify_nonce ($nonce, "s2member-pro-authnet-cancellation"))
							{
								$GLOBALS["ws_plugin__optimizemember_pro_authnet_cancellation_response"] = array (); /* This holds the global response details. */
								$global_response = &$GLOBALS["ws_plugin__optimizemember_pro_authnet_cancellation_response"]; /* This is a shorter reference. */
								/**/
