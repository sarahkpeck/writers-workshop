								...
								if(!get_transient($transient_rtn = "s2m_rtn_".md5("optimizemember_transient_".$_paypal_s)) && set_transient($transient_rtn, time(), 31556926 * 10))
									{
										...
										/**/
										if(/**/(preg_match("/^subscr_payment$/i", $paypal["txn_type"]) && !empty($_GET["optimizemember_paypal_return_tra"])) /* Decrypt/unserialize. */
										...
										&& ($tra["invoice"] === $paypal["invoice"]) && ($tra["checksum"] === md5($paypal["invoice"].$paypal["ip"].$paypal["item_number"]))/**/)
											{
												...
												/**/
												eval('$ipn_signup_vars = $paypal; unset($ipn_signup_vars["optimizemember_log"]);'); /* Create array of wouldbe IPN signup vars w/o optimizemember_log. */
											}
