...
*
* @package optimizeMember\User_Access
* @since 3.5
*/
if (realpath (__FILE__) === realpath ($_SERVER["SCRIPT_FILENAME"]))
	exit ("Do not access this file directly.");
/**/
if (!class_exists ("c_ws_plugin__optimizemember_user_access"))
	{
		/**
		* User access routines.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_user_access
			{
				/**
				...
				public static function user_access_role ($user = FALSE)
					{
						if ((func_num_args () && (!is_object ($user) || empty ($user->ID))) || (!func_num_args () && !$user && (!is_object ($user = (is_user_logged_in ()) ? wp_get_current_user () : false) || empty ($user->ID))))
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_role", "", get_defined_vars ());
							}
						else /* Else we return the first Role in their array of assigned WordPress Roles. */
							return apply_filters ("ws_plugin__optimizemember_user_access_role", reset ($user->roles), get_defined_vars ());
					}
				/**
				...
				public static function user_access_ccaps ($user = FALSE)
					{
						if ((func_num_args () && (!is_object ($user) || empty ($user->ID))) || (!func_num_args () && !$user && (!is_object ($user = (is_user_logged_in ()) ? wp_get_current_user () : false) || empty ($user->ID))))
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_ccaps", array (), get_defined_vars ());
							}
						else /* Otherwise, we DO have the $user object available. */
							{
								...
								/**/
								return apply_filters ("ws_plugin__optimizemember_user_access_ccaps", $ccaps, get_defined_vars ());
							}
					}
				/**
				...
				public static function user_access_level ($user = FALSE)
					{
						if ((func_num_args () && (!is_object ($user) || empty ($user->ID))) || (!func_num_args () && !$user && (!is_object ($user = (is_user_logged_in ()) ? wp_get_current_user () : false) || empty ($user->ID))))
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_level", -1, get_defined_vars ()); /* No $user, or NOT logged in. */
							}
						/**/
						for ($n = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n >= 0; $n--)
							{
								if ($user->has_cap ("access_optimizemember_level" . $n)) /* Testing for Membership Level Access. */
									{
										return apply_filters ("ws_plugin__optimizemember_user_access_level", $n, get_defined_vars ());
									}
							}
						/* Else we assume this is a "User" ( a Free Subscriber with an Access Level of 0. ). */
						return apply_filters ("ws_plugin__optimizemember_user_access_level", 0, get_defined_vars ());
					}
				/**
				...
				public static function user_access_role_to_level ($role = FALSE)
					{
						if (!($role = strtolower ($role))) /* No ``$role`` provided. Default value of -1. */
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_role_to_level", -1, get_defined_vars ());
							}
						else if (in_array ($role, array ("administrator", "editor", "author", "contributor", "bbp_moderator")))
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_role_to_level", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"], get_defined_vars ());
							}
						else if (preg_match ("/^optimizemember_level([0-9]+)$/i", $role, $m) && $m[1] >= 1) /* Test for optimizeMember Roles. */
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_role_to_level", (int)$m[1], get_defined_vars ());
							}
						else if ($role === "subscriber") /* Testing for Free Subscriber Role. */
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_role_to_level", 0, get_defined_vars ());
							}
				...
				public static function user_access_label ($user = FALSE)
					{
						if ((func_num_args () && (!is_object ($user) || empty ($user->ID))) || (!func_num_args () && !$user && (!is_object ($user = (is_user_logged_in ()) ? wp_get_current_user () : false) || empty ($user->ID))))
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_label", "", get_defined_vars ()); /* No $user, or NOT logged in. */
							}
						else if (($level = c_ws_plugin__optimizemember_user_access::user_access_level ($user)) >= 0 && !empty ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $level . "_label"]))
							{
								return apply_filters ("ws_plugin__optimizemember_user_access_label", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $level . "_label"], get_defined_vars ());
							}
						else /* Else there is no Label configured for this User/Member. Return empty string. */
							return apply_filters ("ws_plugin__optimizemember_user_access_label", "", get_defined_vars ());
					}
			}
	}
?>