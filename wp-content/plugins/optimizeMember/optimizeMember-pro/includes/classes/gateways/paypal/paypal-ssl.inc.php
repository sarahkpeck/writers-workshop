...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_paypal_ssl"))
	{
		/**
		* PayPal SSL.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_ssl
			{
				/**
				...
				public static function sc_paypal_form_auto_force_ssl ($force = FALSE, $vars = FALSE)
					{
						...
						if (!$force && defined ("OPTIMIZEMEMBER_PRO_AUTO_FORCE_SSL") && OPTIMIZEMEMBER_PRO_AUTO_FORCE_SSL)
							if (is_object ($post) && strpos ($post->post_content, "[optimizeMember-Pro-PayPal-Form") !== false)
								return ($force = true);
						/**/
