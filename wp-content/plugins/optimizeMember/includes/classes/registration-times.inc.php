...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_registration_times"))
	{
		/**
		* Registration Times.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_registration_times
			{
				/**
				...
				public static function synchronize_paid_reg_times ($user_id = FALSE, $role = FALSE)
					{
						...
						do_action ("ws_plugin__optimizemember_before_synchronize_paid_reg_times", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if ($user_id && is_object ($user = new WP_User ($user_id)) && !empty ($user->ID) && ($level = c_ws_plugin__optimizemember_user_access::user_access_level ($user)) > 0)
							{
				...
				*/
				public static function registration_time ($user_id = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_registration_time", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						if (is_object ($user) && !empty ($user->ID) && ($user_id = $user->ID) && $user->user_registered)
							{
								return apply_filters ("ws_plugin__optimizemember_registration_time", strtotime ($user->user_registered), get_defined_vars ());
							}
						else /* Else we return a default value of 0, because there is insufficient data. */
							return apply_filters ("ws_plugin__optimizemember_registration_time", 0, get_defined_vars ());
					}
				/**
				...
				*/
				public static function paid_registration_time ($level = FALSE, $user_id = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_paid_registration_time", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						$level = (!is_numeric ($level)) ? "level" : "level" . preg_replace ("/[^0-9]/", "", (string)$level);
						$user = ($user_id) ? new WP_User ($user_id) : ((is_user_logged_in ()) ? wp_get_current_user () : false);
						/**/
						if ($level && is_object ($user) && !empty ($user->ID) && ($user_id = $user->ID) && is_array ($pr_times = get_user_option ("optimizemember_paid_registration_times", $user_id)))
							{
								return apply_filters ("ws_plugin__optimizemember_paid_registration_time", ((isset ($pr_times[$level])) ? (int)$pr_times[$level] : 0), get_defined_vars ());
							}
						else /* Else we return a default value of `0`, because there is insufficient data. */
							return apply_filters ("ws_plugin__optimizemember_paid_registration_time", 0, get_defined_vars ());
					}
			}
	}
?>