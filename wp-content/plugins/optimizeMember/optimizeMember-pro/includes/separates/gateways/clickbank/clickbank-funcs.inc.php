...
*/
if (is_dir ($ws_plugin__optimizemember_pro_temp_dir = dirname (dirname (dirname (dirname (__FILE__)))) . "/functions/separates/gateways/clickbank"))
	foreach (scandir ($ws_plugin__optimizemember_pro_temp_dir) as $ws_plugin__optimizemember_pro_temp_s) // Scan all files in this directory.
		if (preg_match ("/\.php$/", $ws_plugin__optimizemember_pro_temp_s) && preg_match ("/^clickbank-/i", $ws_plugin__optimizemember_pro_temp_s))
