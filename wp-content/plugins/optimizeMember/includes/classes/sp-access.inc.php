...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_sp_access"))
	{
		/**
		* Specific Post/Page Access routines.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_sp_access
			{
				/**
				...
				public static function sp_access_link_gen ($sp_ids = FALSE, $hours = 72, $shrink = TRUE)
					{
						...
						do_action ("ws_plugin__optimizemember_before_sp_access_link_gen", get_defined_vars ());
						...
						if ((is_string ($sp_ids) || is_numeric ($sp_ids)) && ($sp_ids = preg_replace ("/[^0-9;,]/", "", $sp_ids)) && ($leading_id = preg_replace ("/^([0-9]+).*$/", "$1", $sp_ids)) && is_numeric ($hours))
							{
								$sp_access = c_ws_plugin__optimizemember_utils_encryption::encrypt ("sp_time_hours:.:|:.:" . $sp_ids . ":.:|:.:" . strtotime ("now") . ":.:|:.:" . $hours);
								/**/
								$sp_access_link = add_query_arg ("optimizemember_sp_access", urlencode ($sp_access), get_permalink ($leading_id)); /* Generate long URL/link. */
								/**/
								if ($shrink && ($shorter_url = c_ws_plugin__optimizemember_utils_urls::shorten ($sp_access_link)))
									$sp_access_link = $shorter_url . "#" . $_SERVER["HTTP_HOST"];
							}
						/**/
						return apply_filters ("ws_plugin__optimizemember_sp_access_link_gen", ((!empty ($sp_access_link)) ? $sp_access_link : false), get_defined_vars ());
					}
				/**
				...
				public static function sp_access ($sp_id = FALSE, $read_only = FALSE)
					{
						...
						/**/
						else if ($sp_id && is_numeric ($sp_id) && ((!empty ($_GET["optimizemember_sp_access"]) && ($_g["optimizemember_sp_access"] = trim (stripslashes ((string)$_GET["optimizemember_sp_access"]))) && is_array ($sp_access_values = array ($_g["optimizemember_sp_access"]))) || is_array ($sp_access_values = c_ws_plugin__optimizemember_sp_access::sp_access_session ())) && !empty ($sp_access_values))
							{
												...
												if (count ($sp_access) === 4 && $sp_access[0] === "sp_time_hours" && in_array ($sp_id, preg_split ("/[\r\n\t\s;,]+/", $sp_access[1])))
													{
														if (is_numeric ($sp_access[2]) && is_numeric ($sp_access[3]) && $sp_access[2] <= strtotime ("now") && ($sp_access[2] + ($sp_access[3] * 3600)) >= strtotime ("now"))
															{
																if (!$read_only && !empty ($_g["optimizemember_sp_access"])) /* Add to session? */
								...
								if (!$read_only && /* A Specific Post/Page Link? */ !empty ($_g["optimizemember_sp_access"]))
									{
				...
				*/
				public static function sp_access_session ($add_sp_access_value = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_sp_access_session", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						$sp_access_values = (!empty ($_COOKIE["optimizemember_sp_access"])) ? preg_split ("/\:\.\:\|\:\.\:/", (string)$_COOKIE["optimizemember_sp_access"]) : array ();
						/**/
						if ($add_sp_access_value && is_string ($add_sp_access_value) && !in_array /* Not in session? */ ($add_sp_access_value, $sp_access_values))
							{
								...
								/**/
								setcookie ("optimizemember_sp_access", $cookie, time () + 31556926, COOKIEPATH, COOKIE_DOMAIN);
								setcookie ("optimizemember_sp_access", $cookie, time () + 31556926, SITECOOKIEPATH, COOKIE_DOMAIN);
								$_COOKIE["optimizemember_sp_access"] = $cookie; /* Real-time cookie updates. */
								/**/
								eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								do_action ("ws_plugin__optimizemember_during_sp_access_session", get_defined_vars ());
								unset ($__refs, $__v); /* Unset defined __refs, __v. */
							}
						/**/
						return apply_filters ("ws_plugin__optimizemember_sp_access_session", $sp_access_values, get_defined_vars ());
					}
			}
	}
?>