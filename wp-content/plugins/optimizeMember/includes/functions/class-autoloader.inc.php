<?php
/**
* optimizeMember class autoloader.
*
* Defines the __autoload function for optimizeMember classes.
* This highly optimizes optimizeMember. Giving it a much smaller footprint.
...
/**/
if (!function_exists ("ws_plugin__optimizemember_classes"))
	{
		/**
		* optimizeMember class autoloader.
		*
		* The __autoload function for optimizeMember classes.
		* This highly optimizes optimizeMember. Giving it a much smaller footprint.
		* See: {@link http://www.php.net/manual/en/function.spl-autoload-register.php}
		*
		* @package optimizeMember
		...
		* @return null
		*/
		function ws_plugin__optimizemember_classes ($class = FALSE) /* Build dynamic __autoload function. */
			{
				...
				/**/
				if (strpos ($class, "c_ws_plugin__optimizemember_") === 0 && strpos ($class, "c_ws_plugin__optimizemember_pro_") === false)
					{
						$c = (!isset ($c)) ? dirname (dirname (__FILE__)) . "/classes" : $c; /* Configures location of classes. */
						$c_class_dirs = (!isset ($c_class_dirs)) ? array_merge (array ($c), _ws_plugin__optimizemember_classes_scan_dirs_r ($c)) : $c_class_dirs;
						/**/
						$class = str_replace ("_", "-", str_replace ("c_ws_plugin__optimizemember_", "", $class));
						/**/
		...
		*
		* Used by the optimizeMember autoloader.
		*
		* @package optimizeMember
		* @since 3.5
		*
		...
		*/
		function _ws_plugin__optimizemember_classes_scan_dirs_r ($starting_dir = FALSE)
			{
					...
					if (is_dir ($starting_dir)) /* Does this directory exist? */
						foreach (scandir ($starting_dir) as $dir) /* Scan this directory. */
							if ($dir !== "." && $dir !== ".." && is_dir ($dir = $starting_dir . "/" . $dir))
								$dirs = array_merge ($dirs, array ($dir), _ws_plugin__optimizemember_classes_scan_dirs_r ($dir));
				/**/
				return $dirs; /* Return array of all directories. */
			}
		/**/
		spl_autoload_register ("ws_plugin__optimizemember_classes"); /* Register __autoload. */
	}
?>