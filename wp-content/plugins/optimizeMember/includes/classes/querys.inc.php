												...
												if((is_user_logged_in() && is_object($user = wp_get_current_user()) && !empty($user->ID) && ($user_id = $user->ID)) || !($user = false))
													{
														...
														unset /* A little housekeeping here. Ditch these temporary variables. */($_lwp, $_dep, $_ccaps, $_sps, $_drips);
														/**/
														...
														for($n = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n >= 0; $n--) /* Post Level Restrictions. */
															{
																if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_posts"] === "all" && (!$user || !current_user_can("access_optimizemember_level".$n)))
																...
																else if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_posts"] && (!$user || !current_user_can("access_optimizemember_level".$n)))
																	{
																		...
																		/**/
																		$_posts = array_unique( /* Force integers. */c_ws_plugin__optimizemember_utils_arrays::force_integers($_posts));
																		/**/
							...
							if((!is_admin() && ($wp_query->is_search() || $wp_query->is_feed())) || c_ws_plugin__optimizemember_querys::_is_admin_ajax_search($wp_query))
								{
									...
									$s = $systematics = c_ws_plugin__optimizemember_utils_arrays::force_integers /* Force integer values here. */($s);
									/**/
						...
						if(is_array($items) && !empty($items) && is_object($wpdb) && is_object($wp_query) && $wp_query->get("suppress_filters") !== "n/a")
							{
								...
								$x_post_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers /* Force integer values here. */($x_post_ids);
								/**/
								$x_taxonomy_ids = array_merge((array)$wp_query->get("category__not_in"), (array)$wp_query->get("tag__not_in"));
								$x_taxonomy_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers /* Force integer values here. */($x_taxonomy_ids);
								/**/
						...
						if(is_string($cwhere) && is_object($wpdb) && is_object($wp_query) && !$wp_query->get("suppress_filters"))
							{
								...
								$x_terms = array_unique(c_ws_plugin__optimizemember_utils_arrays::force_integers /* Force integer values here. */($x_terms));
								$x_singulars = /* Singulars. */ c_ws_plugin__optimizemember_utils_gets::get_singular_ids_in_terms($x_terms);
								/**/
