<?php
/**
* Menu page for optimizeMember Pro ( Coupon Codes page ).
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_menu_page_coupon_codes"))
	{
		/**
		* Menu page for optimizeMember Pro ( Coupon Codes page ).
		*
		...
		* @since 110531
		*/
		class c_ws_plugin__optimizemember_pro_menu_page_coupon_codes
			{
				public function __construct ()
					{
						...
						echo '<h2>Pro Form ( Coupon Codes )</h2>' . "\n";
						/**/
                        ...
                        if(in_array('stripe', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_gateways_enabled']))
                        {
                            echo '<div class="info">'."\n";
                            echo '<p><strong>* Stripe Coupons (PLEASE NOTE):</strong> coupons MUST be configured with OptimizeMember and <strong>NOT via the Stripe Dashboard please</strong>. OptimizeMember Pro Forms currently do NOT support Stripe-generated coupon codes; only those configured here with OptimizeMember. Please see configuration options below.</p>'."\n";
                            echo '</div>'."\n";
                        }

						echo '<p><strong>Prerequisites:</strong> In order to display a "Coupon Code" field on your Checkout Form, you MUST add this special Shortcode Attribute to your optimizeMember Pro Form Shortcode(s): <code>accept_coupons="1"</code>. If you would like to force-feed a default Coupon Code <em>( optional )</em>, you can add this special Shortcode attribute: <code>coupon="[your default code]"</code>. Also optional, instead of <code>coupon="[your default code]"</code>, you could pass <code>?s2p-coupon=[your default code]</code> in the query string of a URL leading to a Checkout Form.</p>' . "\n";
						/**/
						...
						echo '<td>' . "\n";
						/**/
						echo '<textarea name="ws_plugin__optimizemember_pro_coupon_codes" id="ws-plugin--optimizemember-pro-coupon-codes" rows="10" wrap="off" spellcheck="false" style="width:99%;">' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_coupon_codes"]) . '</textarea><br />' . "\n";
						/**/
						...
						echo '</ul>' . "\n";
						/**/
						echo '<em>* optimizeMember will NEVER allow the Regular Billing Rate ( or Total ) to be less than: <strong>$0.01</strong>.<br />' . "\n";
						...
						echo '<div class="ws-menu-page-hr"></div>' . "\n";
						/**/
						echo '<em>By default, optimizeMember will apply the discount to ALL amounts, including any Regular/Recurring fees.<br />' . "\n";
						...
						echo '</ul>' . "\n";
						/**/
						echo '<em>* As noted above, optimizeMember will NEVER allow the (ra) Regular Amount to be less than: <strong>$0.01</strong>.<br />' . "\n";
						echo 'However, optimizeMember WILL allow the (ta) Trial Amount to be discounted all the way down to <strong>$0.00</strong>.</em>' . "\n";
						/**/
						echo '<div class="ws-menu-page-hr"></div>' . "\n";
						/**/
						echo '<em>By default, optimizeMember accepts Coupon Codes on any Pro Form with Shortcode Attribute: <code>accept_coupons="1"</code>.<br />' . "\n";
						...
						echo '<em>* optimizeMember ONLY accepts Coupons on Pro Forms with Shortcode Attribute: <code>accept_coupons="1"</code></em>' . "\n";
						/**/
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						...
						/**/
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-affiliate-coupon-code-suffix-chars">' . "\n";
						...
						/**/
						echo '<td>' . "\n";
						echo '<input type="text" autocomplete="off" name="ws_plugin__optimizemember_pro_affiliate_coupon_code_suffix_chars" id="ws-plugin--optimizemember-pro-affiliate-coupon-code-suffix-chars" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_affiliate_coupon_code_suffix_chars"]) . '" /><br />' . "\n";
						echo 'Characters that optimizeMember will use to identify an Affiliate\'s ID in Coupon Codes.<br />' . "\n";
						echo '<em>Example: <code>COUPON-CODE' . esc_html ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_affiliate_coupon_code_suffix_chars"]) . '123</code>. Coupon Code is <code>COUPON-CODE</code>. Affiliate ID is <code>123</code>.</em>' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						echo '<tr>' . "\n";
						/**/
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-affiliate-coupon-code-tracking-urls">' . "\n";
						...
						echo '<textarea name="ws_plugin__optimizemember_pro_affiliate_coupon_code_tracking_urls" id="ws-plugin--optimizemember-pro-affiliate-coupon-code-tracking-urls" rows="3" wrap="off">' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_affiliate_coupon_code_tracking_urls"]) . '</textarea><br />' . "\n";
						...
						echo '<em>To actually credit your affiliates, please check your Dashboard here: <code>optimizeMember -> API / Tracking (or API / Notifications)</code>.</em><br /><br />' . "\n";
						echo '<strong>You can use these special Replacement Codes in your Tracking URL(s), as needed.</strong>' . "\n";
						echo '<ul>' . "\n";
						echo '<li><code>%%full_coupon_code%%</code> = The full Affiliate Coupon Code accepted by optimizeMember.</li>' . "\n";
						echo '<li><code>%%coupon_code%%</code> = The actual Coupon Code accepted by your configuration of optimizeMember.</li>' . "\n";
						...
						echo '<li><code>IMG-1PX|URL goes here</code> = Coupon Code Tracking takes place in a potential Customer\'s browser, through a 1px IMG tag, usually with Cookies set by your affiliate tracking system. This is the most compatible across various affiliate software applications. You give optimizeMember the Tracking URL, and optimizeMember will load the 1px IMG tag at the appropriate time.</li>' . "\n";
						echo '</ul>' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
...
/**/
new c_ws_plugin__optimizemember_pro_menu_page_coupon_codes ();
?>