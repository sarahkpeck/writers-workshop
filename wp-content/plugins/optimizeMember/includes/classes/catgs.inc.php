<?php
/**
* optimizeMember's Category protection routines *( for current page )*.
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_catgs"))
	{
		/**
		* optimizeMember's Category protection routines *( for current page )*.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_catgs
			{
				/**
								...
								if (!c_ws_plugin__optimizemember_systematics::is_wp_systematic_use_page ()) /* Do NOT touch WordPress Systematics. This excludes all WordPress Systematics. */
									{
										...
										if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["login_redirection_override"] && ($login_redirection_uri = c_ws_plugin__optimizemember_login_redirects::login_redirection_uri ($user, "root-returns-false")) && preg_match ("/^" . preg_quote ($login_redirection_uri, "/") . "$/", $_SERVER["REQUEST_URI"]) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level0")))
											c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("catg", $cat_id, "level", 0, $_SERVER["REQUEST_URI"], "sys") . exit ();
										/**/
												...
												for ($n = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n >= 0; $n--) /* Category Level restrictions. Go through each Level. We also check nested Categories. */
													{
														if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_catgs"] === "all" && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level" . $n)))
															c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("catg", $cat_id, "level", $n, $_SERVER["REQUEST_URI"]) . exit ();
														/**/
														...
														if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_ruris"]) /* URIs configured at this Level? */
															...
															foreach (preg_split ("/[\r\n\t]+/", c_ws_plugin__optimizemember_ruris::fill_ruri_level_access_rc_vars ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_ruris"], $user)) as $str)
																if ($str && preg_match ("/" . preg_quote ($str, "/") . "/", $_SERVER["REQUEST_URI"]) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level" . $n)))
																	c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("catg", $cat_id, "level", $n, $_SERVER["REQUEST_URI"], "ruri") . exit ();
													}
											}
										/**/
