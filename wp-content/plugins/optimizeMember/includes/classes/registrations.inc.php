				...
				public static function generate_password ($password = FALSE)
					{
						...
						/**/
						if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_password"] || (c_ws_plugin__optimizemember_utils_conds::pro_is_installed () && c_ws_plugin__optimizemember_pro_remote_ops::is_remote_op ("create_user")))
							{
								if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_password"] && !empty ($_POST["ws_plugin__optimizemember_custom_reg_field_user_pass1"]))
									{
										if (($custom = trim (stripslashes ((string)$_POST["ws_plugin__optimizemember_custom_reg_field_user_pass1"]))))
											$password = $custom; /* Yes, use optimizeMember custom Password supplied by User. */
									}
								else if (c_ws_plugin__optimizemember_utils_conds::pro_is_installed () && c_ws_plugin__optimizemember_pro_remote_ops::is_remote_op ("create_user") && !empty ($GLOBALS["ws_plugin__optimizemember_generate_password_return"]))
									{
										if (($custom = trim (stripslashes ((string)$GLOBALS["ws_plugin__optimizemember_generate_password_return"]))))
											$password = $custom; /* Yes, use optimizeMember custom Password supplied by Remote Op. */
									}
							}
						/**/
						return apply_filters ("ws_plugin__optimizemember_generate_password", ($GLOBALS["ws_plugin__optimizemember_generate_password_return"] = $password), get_defined_vars ());
					}
				/**
												...
												if ($user_id && is_object ($user = new WP_User ($user_id)) && !empty ($user->ID) && ($user_id = $user->ID) && ($processed = true))
													{
														...
														/**/
														else if (!is_admin () && (!c_ws_plugin__optimizemember_utils_conds::pro_is_installed () || !c_ws_plugin__optimizemember_pro_remote_ops::is_remote_op ("create_user")))
															{ /* This routine could be processed through `/wp-login.php?action=register`, `/wp-activate.php`, or `/activate` via BuddyPress`.
																	This may also be processed through a standard BuddyPress installation, or another plugin calling `user_register`.
																	If processed through `/wp-activate.php`, it could've originated inside the admin, via `/user-new.php`. */
																/**/
