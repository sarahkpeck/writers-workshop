...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_mo_page_in"))
	{
		/**
		* Membership Options Page ( inner processing routines ).
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_mo_page_in
			{
				/**
						...
						if (!empty ($_GET["optimizemember_membership_options_page"]) && is_array ($_g = c_ws_plugin__optimizemember_utils_strings::trim_deep (stripslashes_deep ($_GET))))
							{
								...
								/**/
								foreach ($_g as $var => $value) /* Include all of the `_?optimizemember_` variables. */
				...
				*/
				public static function wp_redirect_w_mop_vars ($seeking_type = FALSE, $seeking_type_value = FALSE, $req_type = FALSE, $req_type_value = FALSE, $seeking_uri = FALSE, $res_type = FALSE)
					{
						do_action ("ws_plugin__optimizemember_before_wp_redirect_w_mop_vars", get_defined_vars ());
						/**/
						$status = /* Allow Filters. Defaults to `301`. */ apply_filters ("ws_plugin__optimizemember_content_redirect_status", 301, get_defined_vars ());
						$status = /* Allow Filters. Defaults to `301`. */ apply_filters ("ws_plugin__optimizemember_wp_redirect_w_mop_vars_status", $status, get_defined_vars ());
						/**/
						...
						/**/
						wp_redirect (add_query_arg (urlencode_deep (array ("_optimizemember_seeking" => array ("type" => (string)$seeking_type, urlencode ((string)$seeking_type)=> (string)$seeking_type_value, "_uri" => (string)$seeking_uri), "_optimizemember_req" => array ("type" => (string)$req_type, urlencode ((string)$req_type)=> (string)$req_type_value), "_optimizemember_res" => array ("type" => (string)$res_type), "optimizemember_seeking" => (string)$seeking_type . "-" . (string)$seeking_type_value, "optimizemember_" . urlencode ((string)$req_type) . "_req" => (string)$req_type_value)), get_page_link ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_options_page"])), $status);
						/**/
						do_action ("ws_plugin__optimizemember_after_wp_redirect_w_mop_vars", get_defined_vars ());
						/**/
