...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_alipay_utilities"))
	{
		/**
		* AliPay utilities.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_alipay_utilities
			{
				/**
				...
				public static function alipay_link_gen ($vars = FALSE)
					{
						...
						/**/
						foreach (c_ws_plugin__optimizemember_utils_strings::trim_deep ($vars) as $var => $value)
						...
						/**/
						return $gateway . "?" . $query . "&sign=" . urlencode (md5 ($_q . $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_alipay_security_code"])) . "&sign_type=MD5";
					}
				/**
						...
						if (!empty ($_REQUEST["notify_id"]) && !empty ($_REQUEST["notify_type"]) && preg_match ("/^trade_status_sync$/i", $_REQUEST["notify_type"]) && !empty ($_REQUEST["sign"]))
							{
								...
								foreach ($postvars as $var => $value)
									if ($var && strlen ($value) && !preg_match ("/^(sign|sign_type)$/", $var))
										$_q .= (($_q) ? "&" : "") . $var . "=" . $value;
								/**/
								if ($postvars["sign"] === md5 ($_q . $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_alipay_security_code"])/**/
								&& preg_match ("/true$/i", trim (c_ws_plugin__optimizemember_utils_urls::remote ($gateway . "?service=notify_verify&partner=" . urlencode ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_alipay_partner_id"]) . "&notify_id=" . urlencode ($postvars["notify_id"]), "", array ("timeout" => 20)))))
									return $postvars;
								/**/
								else /* Nope. */
									return false;
