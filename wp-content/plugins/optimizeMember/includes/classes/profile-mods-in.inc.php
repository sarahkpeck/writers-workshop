<?php
/**
* optimizeMember Profile modifications ( inner processing routines ).
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_profile_mods_in"))
	{
		/**
		* optimizeMember Profile modifications ( inner processing routines ).
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_profile_mods_in
			{
				/**
						...
						if (!empty ($_POST["ws_plugin__optimizemember_profile_save"]) && is_user_logged_in () && is_object ($user) && !empty ($user->ID) && ($user_id = $user->ID))
							{
								if (($nonce = $_POST["ws_plugin__optimizemember_profile_save"]) && wp_verify_nonce ($nonce, "ws-plugin--optimizemember-profile-save"))
									{
										$GLOBALS["ws_plugin__optimizemember_profile_saved"] = true; /* Global flag as having been saved/updated successfully. */
										/**/
