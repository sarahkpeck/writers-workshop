...
jQuery(document).ready (function($)
	{
		...
		if (location.href.match (/page\=ws-plugin--optimizemember-pro-clickbank-ops/))
			{
		...
		else if (location.href.match (/page\=ws-plugin--optimizemember-pro-clickbank-buttons/))
			{
				$('div.ws-menu-page select[id]').filter ( /* Filter all select elements with an id. */function()
					{
						return this.id.match (/^ws-plugin--optimizemember-pro-level[1-9][0-9]*-type$/);
					}).change (function()
					{
						var button = this.id.replace (/^ws-plugin--optimizemember-pro-(.+?)-type$/g, '$1');

						var termDisabled = ($(this).val () === 'recurring') ? true : false;
						var periodsDisabled = ($(this).val () === 'standard') ? true : false;

						$('p#ws-plugin--optimizemember-pro-' + button + '-term-line').css ('display', (termDisabled ? 'none' : ''));
						$('p#ws-plugin--optimizemember-pro-' + button + '-periods-line').css ('display', (periodsDisabled ? 'none' : ''));

						(termDisabled) ? $('select#ws-plugin--optimizemember-pro-' + button + '-term').val ('1-M') : null;
						(periodsDisabled) ? $('select#ws-plugin--optimizemember-pro-' + button + '-p1').val ('0-D') : null;
						(periodsDisabled) ? $('select#ws-plugin--optimizemember-pro-' + button + '-p3').val ('1-M') : null;
				...
				ws_plugin__optimizemember_pro_clickbankButtonGenerate = /* Handles ClickBank® Button Generation. */ function(button)
					{
						var shortCodeTemplate = '[optimizeMember-Pro-ClickBank-Button %%attrs%% image="default" output="anchor" /]', shortCodeTemplateAttrs = '', labels = {};

						var vendor = '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_username"]); ?>';
						...
						var prodType = $('select#ws-plugin--optimizemember-pro-' + button + '-type').val ().replace (/[^A-Z]/gi, '').toLowerCase ();
						var prodItem = $('input#ws-plugin--optimizemember-pro-' + button + '-item-number').val ().replace (/[^0-9]/g, '');
						...
						if /* In this case, we used the Fixed-Term configuration. */ (prodType === 'standard')
							...
							{
								var trialPeriod = $('select#ws-plugin--optimizemember-pro-' + button + '-p1').val ().split ('-')[0].replace (/[^0-9]/g, '');
								var trialTerm = $('select#ws-plugin--optimizemember-pro-' + button + '-p1').val ().split ('-')[1].replace (/[^A-Z]/g, '');
								var regPeriod = $('select#ws-plugin--optimizemember-pro-' + button + '-p3').val ().split ('-')[0].replace (/[^0-9]/g, '');
								var regTerm = $('select#ws-plugin--optimizemember-pro-' + button + '-p3').val ().split ('-')[1].replace (/[^A-Z]/g, '');
						...
						shortCodeTemplateAttrs += 'cbp="' + esc_attr(prodItem) + '" cbskin="" cbfid="" cbur="" cbf="auto" level="' + esc_attr(level) + '" ccaps="' + esc_attr(cCaps) + '" desc="' + esc_attr(desc) + '" custom="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["HTTP_HOST"])); ?>"';
				...
				ws_plugin__optimizemember_pro_clickbankCcapButtonGenerate = /* Handles ClickBank® Button Generation. */ function()
					{
						var shortCodeTemplate = '[optimizeMember-Pro-ClickBank-Button %%attrs%% image="default" output="anchor" /]', shortCodeTemplateAttrs = '';
						...
						var prodType = $('select#ws-plugin--optimizemember-pro-ccap-type').val ().replace (/[^A-Z]/gi, '').toLowerCase ();
						var prodItem = $('input#ws-plugin--optimizemember-pro-ccap-item-number').val ().replace (/[^0-9]/g, '');
						...
						shortCodeTemplateAttrs += 'cbp="' + esc_attr(prodItem) + '" cbskin="" cbfid="" cbur="" cbf="auto" level="*" ccaps="' + esc_attr(cCaps) + '" desc="' + esc_attr(desc) + '" custom="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["HTTP_HOST"])); ?>"';
				...
				ws_plugin__optimizemember_pro_clickbankSpButtonGenerate = /* Handles ClickBank® Button Generation for Specific Post/Page Access. */ function()
					{
						var shortCodeTemplate = '[optimizeMember-Pro-ClickBank-Button %%attrs%% image="default" output="anchor" /]', shortCodeTemplateAttrs = '';
						...
						var prodItem = $('input#ws-plugin--optimizemember-pro-sp-item-number').val ().replace (/[^0-9]/g, '');
						...
						shortCodeTemplateAttrs += ' custom="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["HTTP_HOST"])); ?>"';
						shortCode.val (shortCodeTemplate.replace (/%%attrs%%/, shortCodeTemplateAttrs));
				...
				ws_plugin__optimizemember_pro_clickbankRegLinkGenerate = /* Handles ClickBank® Link Generation. */ function()
					{
				...
				ws_plugin__optimizemember_pro_clickbankSpLinkGenerate = /* Handles ClickBank® Link Generation. */ function()
					{
