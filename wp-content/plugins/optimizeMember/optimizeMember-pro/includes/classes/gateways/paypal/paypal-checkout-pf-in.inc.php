		...
		class c_ws_plugin__optimizemember_pro_paypal_checkout_pf_in
			{
				...
				*/
				public static function paypal_checkout()
					{
						if(/**/(/**/!empty($_POST["optimizemember_pro_paypal_checkout"]["nonce"]) && ($nonce = $_POST["optimizemember_pro_paypal_checkout"]["nonce"]) && wp_verify_nonce($nonce, "s2member-pro-paypal-checkout")/**/)/**/
						...
						&& ($xco_post_vars = get_transient("s2m_".md5("optimizemember_transient_express_checkout_".$_GET["token"])))/**/)/**/)
							{
								...
								$post_vars = c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($post_vars)); /* And Filter. */
										...
										if(!($error = c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_submission_validation_errors("checkout", $post_vars)))
											{
												...
												/**/
													...
													{
														if(!($cur__subscr_id = get_user_option("optimizemember_subscr_id")) || !($paypal = c_ws_plugin__optimizemember_pro_paypal_utilities::payflow_get_profile($cur__subscr_id)) || !preg_match("/^(Pending|PendingProfile)$/i", $paypal["STATUS"]))
															{
																...
																if(!($paypal = array()) /* Recurring Profile. */)
																	{
																		...
																		/**/
																		if($_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" && !empty($_GET["token"]) && ($paypal_xco_details = array("TRXTYPE" => "S", "TENDER" => "P", "ACTION" => "G", "TOKEN" => $_GET["token"])) && ($paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($paypal_xco_details)) && empty($paypal_xco_details["__error"]))
																			{
																				$paypal["TENDER"] = "P";
																...
																if(($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($paypal)) && empty($paypal["__error"]))
																	{
																		...
																		if(!($paypal = array()) && $old__subscr_id)
																			c_ws_plugin__optimizemember_pro_paypal_utilities::payflow_cancel_profile($old__subscr_id);
																		/**/
